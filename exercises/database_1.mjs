...
export class Database {
  ...
  execute(statement) {
    // Perceba que o código abaixo só funciona pois estamos usando arrow functions ao invés de functions na callback do new Promise e do setTimeout, uma vez que as arrow functions usam o 'this' herdado do contexto de execução, ao passo que as funções possuem um 'this' próprio.
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const result = this.parser.parse(statement);

        if (result) {
          // O resolve não para a execução do código como uma cláusula 'return', portanto, o código abaixo do if statement será executado normalmente caso o return não esteja presente.
